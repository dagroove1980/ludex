...
export async function processGameRules(gameTitle, pdfText) {
  if (!genAI) {
    throw new Error('GEMINI_API_KEY environment variable is not set. Please add it in Vercel Settings → Environment Variables.');
  }
  
  // Try different model names - the API version determines which models are available
  ...
  if (!model) {
    throw new Error(`None of the models (${modelNames.join(', ')}) are available. Please check your GEMINI_API_KEY and API access.`);
  }
...
export async function chatAboutGame(gameTitle, sections, strategyTips, quickStart, conversationHistory, userMessage) {
  ...
  if (!model) {
    throw new Error(`None of the models (${modelNames.join(', ')}) are available.`);
  }
